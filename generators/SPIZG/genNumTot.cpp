...
double rand_num(int i){
  //Standard GSL random number generator
  ...
  //This random number generator produces an integer. For the purposes of my generator, I want random numbers between 0 and 1 with good prescision
  //To to this I am producing a random number between 0 and 1000 and then dividing by 1000
  double first = 0, second = 1000;

  //produce random number
  double randnum = gsl_rng_uniform(r);
  double NumInRange = randnum * (second - first) + first;

  //free variables so the function can be called again
  gsl_rng_free(r);

  return NumInRange/1000;
}
...
void genNumTot(){
  ...
  //output file
  TFile *fout = TFile::Open("OUTPUT/monteInputHistImTest2.root", "RECREATE");
  if (!f1) { return; }
  ...
  for(int k = 0; k < MAXEVENTS; k++){ //For loop is capped at 1000x the desired number of events    
    if(evtsSoFar >  SPIZG_NUMEVENTS) continue; //Stop doing anything once the desired number of events is reached 
    accVal = rand_num(k)*toUMax;               //Accept value for primakoff cross-section 

    randE2 = rand_num(k)*(maxE - minE) + minE; //Random photon energy value
    ...
    randTheta = 2*rand_num(k);             //Produce random number in theta 
    ...
    randPhi = Gmin(rand_num(k), SPIZG_POL_FRAC, SPIZG_POL_DEG); //Produce values of phi with distribution of NPP equation 
    phiPi0->Fill(randPhi);
    ...
    evtsSoFar += 1; //For getting exactly the right number of events (+ 1) 
    //if(evtsSoFar >  SPIZP_NUMEVENTS) continue;
  }
