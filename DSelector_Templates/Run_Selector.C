...
int Run_Selector(string locInputFileName, string locTreeName, string locSelectorName, unsigned int locNThreads)
{
	...
	//process tree directly - use TChain to handle wildcards or multiple files
	TChain* locChain = new TChain(locTreeName.c_str());
	locChain->Add(locInputFileName.c_str());
	cout << "Total entries in chain: " << locChain->GetEntries() << " from " << locChain->GetNtrees() << " file(s)" << endl;
